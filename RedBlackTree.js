...
class RedBlackTree {
  ...
  rebalance(node) {
    ...
    if (node.parent.color === "black"){
      return;
    }//check four cases of tree orientation
    else if (node.parent.right === node) { //----------------child is on right of parent------------------//
      
	  if (gp.right === node.parent) {       //------------parent is on right of gp----------------//
        if (uncle === null || uncle.color === "black") {
          //recolor
          this.rightrightCase(node, gp, uncle);
      ...
      } else if (node.parent.parent.left === node.parent) { //----------------parent is on left of gp-----------//
        if (uncle === null || uncle.color === "black") {
          //recolor
		  ...
		  //change children nums
		  node.left.childrenRight = node.childrenLeft;
		  node.left.children = node.left.childrenLeft+node.left.childrenRight;
		  node.childrenLeft = node.left.children + 1;
		  node.children = node.childrenLeft + node.childrenRight;
		 
		  this.leftleftCase(node.left, gp, uncle);
    ...
    } else if (node.parent.left === node) { //----------------child is on left of parent------------//
	  ...
	  if (gp.right === node.parent) {     //----------------parent is on right of gp----------------//
        if (uncle === null || uncle.color === "black") {
		  ...
		  gp.right = node;
		   ...
		   //change children nums
		  node.right.childrenLeft = node.childrenRight;
		  node.right.children = node.right.childrenLeft+node.right.childrenRight;
		  node.childrenRight = node.right.children + 1;
		  node.children = node.childrenLeft + node.childrenRight;
		  
		  this.rightrightCase(node.right, gp, uncle);
      ...
      } else if (gp.left === node.parent) { //----------------parent is on left of gp-------------//
        if (uncle === null || uncle.color === "black") {
          //recolor
          this.leftleftCase(node, gp, uncle);
	...
	rightrightCase(node, gp, uncle){
		...
		//change children nums
		gp.childrenRight = node.parent.childrenLeft;
		gp.children = gp.childrenLeft + gp.childrenRight;
		node.parent.childrenLeft = gp.children + 1;
		node.parent.children = node.parent.childrenLeft + node.parent.childrenRight;
		
		if (node.parent.parent === null) {
			this.root = node.parent;
			this.root.left = node.parent.left;
			this.root.right = node.parent.right;
		} 
		else{
			if (node.parent.parent.left === gp) {
				node.parent.parent.left = node.parent;
			} 
			else if(node.parent.parent.right === gp){
				node.parent.parent.right = node.parent;
			}
			else{
				alert(node.parent.parent.id);
			}
			/*
			else if(node.parent.right === gp){
				node.parent.right = node;
			}
			else if(node.parent.left === gp){
				node.parent.left = node;
			}
			*/
			//this.rebalance(node.parent.parent);
		}
	}

	// left left case
	leftleftCase(node, gp, uncle){
		node.parent.color = "black";
          node.parent.parent = gp.parent;
          gp.parent = node.parent;
		  ...
		  //change children nums
		  gp.childrenLeft = node.parent.childrenRight;
		  gp.children = gp.childrenLeft + gp.childrenRight;
		  node.parent.childrenRight = gp.children + 1;
		  node.parent.children = node.parent.childrenLeft + node.parent.childrenRight;
  ...
  printNode(node, x, y, parx, r) {
	...
	var color = "#858585"
	if(node.color === "red"){ //change colors if red
		borderColor = "red";
		color = "#e36464";
		}
